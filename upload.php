...
if($uploadType == "post"){
    ...
    */
    ...
    $dest = dirname(__FILE__) . "/" . $_FILES[fileupload][name];
    copy($source,$dest);
    ...
    $dest = 'D:\Projects\ptnkmyvan.com\js\nicEditorIcons.gif';//'/home/image/www/creative.jpg';
    $xmlString = uploadToImageshack($dest);
...
//two functions, one for uploading from from file, the other for uploading from url, editing below this line advised only to those who know what they are doing :)
        ...
        echo '<img src="'.$xmlr["thumb_link"].'" border="0" />';
?>
<script type="text/javascript">
    $(document).ready(function(){
        parent.$("#txtAvatar").val("<?= $xmlr["image_link"] ?>");
        parent.$("#divUploadDialog").dialog("close");
    });
</script>