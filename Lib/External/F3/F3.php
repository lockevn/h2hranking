...
final class F3 {
	...
	//! Framework version
	const TEXT_Version='PHP Fat-Free Framework 1.2.10';
	...
	**/
	public static function serve($_file,$_ishtml=TRUE,$_path=NULL) {
		...
		$_docreg='/<\!DOCTYPE\s+html.*?>/is';
		$_xmlreg='/<\?xml.*\?>/is';
		$_doctag=preg_match($_docreg,$_contents);
		$_xmltag=preg_match($_xmlreg,$_contents);
		...
		if ($_ishtml) {
			// HTML template
			$_contents=self::resolve(
				html_entity_decode($_tree->saveHTML()));
			$_tree->loadHTML($_contents);
		}
		else {
			// XML template
			$_contents=self::resolve(
				self::xmlDecode($_tree->saveXML()));
			$_tree->loadXML($_contents);
		}
		if ($_tree->getElementsByTagName('check')->length) {
		...
		if ($_ishtml) {
			// Correct empty tags
			$_contents=preg_replace('/<((?:area|base|br|col|frame|hr|'.
				'img|input|isindex|link|meta|param).*?)(?!\/)>/','<$1/>',
				$_tree->saveHTML());
			if (!$_xmltag)
				$_contents=preg_replace($_xmlreg,'',$_contents);
			if (!$_doctag)
				$_contents=preg_replace($_docreg,'',$_contents);
		}
		else
			$_contents=$_tree->saveXML();
		return $_contents;
	}
